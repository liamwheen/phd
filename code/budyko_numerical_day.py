...
class Budyko:
    ...
    def iter_func(self):
        ...
        for frame, t in enumerate(self.t_span):
            ...
            #plt.figure()
            #plt.plot(self.Qs(self.y_span)*(1-self.a_eta(self.y_span)))
            #plt.plot((A + B*self.T))
            #plt.plot(C*(self.T - self.int_T(self.T)))
            #plt.show()
            self.T, self.eta = X[:-1],min(X[-1],1)
    ...
    def update(self, t):
        ...
        self.insol.set_ydata(self.Qs(self.y_span)/7-40)
