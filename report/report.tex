...
parameter values, this yields 2 equilibrium points within the valid range.}
...
\end{figure}
---Rework this, write up the land albedo stuff, maybe do some changes to the
Budyko model before producing the time series/spectrogram, maybe try out a
varying albedo based on angle.----
...
The albedo value of 0.32 was determined from observational data, incorporating the effect of both
land and cloud cover. Hence, although the model assumes Earth's surface to
be entirely ocean, this is not applied to the calculation of albedo.
By representing the surface albedo as homogeneous, the different
...
Yang showed that the albedo increases approximately two times when ZSA goes
from 0$^\circ$ to 90$^\circ$ \cite{land_zenith_range}.
...
back out to space is equal to $0.29Q_\varepsilon=98.71$\,Wm$^{-2}$.

The SZA is calculated with the same formula used in calculating
insolation at a specific point on Earth, given as
\[
  S(\theta,\beta,\rho,\varphi,\gamma) = (\sin{\gamma}\sin{(\rho -
  \theta)}-\cos{\beta} \cos{\gamma} \cos{(\rho-\theta)})\cos{\varphi}
  -\sin{\beta}\sin{\varphi} \cos{(\rho -\theta)}.
...
The albedo is approximated by fitting a quadratic function
to the data collected by Jin \cite{albedo_zenith_effect} on ocean albedo for
varied SZA. Since the SZA depends on theta, the approximation
for yearly average insolation that has been used so far is no longer
sufficient. We must therefore implement a numerical approximation to the yearly
insolation which applies the albedo mask over the daily insolation before
averaging this over the year. --NOT SURE WHY THIS IS
NECESSARY YET----
