...
$Q_\varepsilon s(y) (1-\alpha(y,\eta))$. This consists of the average yearly insolation reaching
...
This is finally multiplied by $(1-\alpha(y,\eta))$, which describes the degree to which heat is
absorbed at a given latitude dependent on the location of the ice line where
the albedo function is given as
\[
  \alpha(y,\eta) = \begin{cases} \alpha_1, & y<\eta\\
                                 \alpha_2, & y>\eta\\
                                 \frac{1}{2}(\alpha_1+\alpha_2), & y=\eta
                   \end{cases},
\]
where $\alpha_1$ and $\alpha_2$ are also determined through empirical data.
This results in less heat being absorbed over the range where ice is present
...
\[
  - (A+BT(y,t)),
\]
where the minus sign indicates the energy is leaving Earth.
Although the energy flow occurring within Earth's atmosphere is far more
complex, empirical data was used to establish the net flow of heat from Earth
...
the hot equator to the relatively cooler pole. The direction of flow at a given
latitude is determined by its difference to the global average temperature.
...
\]
where $A, B, C,$ and $R$ are constants found using empirical data.
...
Figure \ref{fig:orbital_angles} shows Earth's orbit around the sun with $\psi$
indicating 5\% of the orbit either side of the solstices. The eccentricity
...
$\theta_{\mathrm{S}}=\rho$. From (\ref{eq:dtheta_dt}), it is clear that it will
take far longer for Earth to orbit the range $[\theta_{\mathrm{N}}-\frac{\psi}{2},
\theta_{\mathrm{N}}+\frac{\psi}{2}]$ than the southern hemisphere equivalent.
...
\begin{figure}
  ...
  the sun (yellow) with $\psi$ indicating a 5\% range either side of the
...
the orbit, at latitude $-\varphi$. This results in the annual average for
insolation being symmetric about the equator.

To express insolation over the orbital range, $\psi$, we integrate over
$\theta$, but also the longitude, $\gamma$. Recall that the limits for $\gamma$
vary according to the Earth's orientation to the sun such that the integration
only applies to the positive part of the insolation function where it is valid.
The insolation over the given ranges is therefore
\[
  Q = \int_{t(\theta_{\mathrm{N}}-\frac{\psi}{2})}^{t(\theta_{\mathrm{N}}+\frac{\psi}{2})}
  \int_{\gamma_1}^{\gamma_2}
  \frac{K}{4\pi r^2} I(r,\theta,\beta,\rho,\varphi,\gamma)\, \mathrm{d}\gamma\,\mathrm{d}t,
\]
...
\[
  ...
  \int_{\gamma_1}^{\gamma_2}
  \frac{K}{4\pi r^2}I(\theta,\beta,\rho,\varphi,\gamma)\,\mathrm{d}\gamma\,\frac{\mathrm{d}t}{\mathrm{d}\theta}\,\mathrm{d}\theta\\
  &=\int_{\theta_{\mathrm{N}}-\frac{\psi}{2}}^{\theta_{\mathrm{N}}+\frac{\psi}{2}}\int_{\gamma_1}^{\gamma_2}
  \frac{K}{4\pi r^2}\frac{Pr^2}{2\pi a b}I(\theta,\beta,\rho,\varphi,\gamma)\, \mathrm{d}\gamma\,\mathrm{d}\theta\\
  &=\frac{KP}{8\pi^2 a b}\int_{\theta_{\mathrm{N}}-\frac{\psi}{2}}^{\theta_{\mathrm{N}}+\frac{\psi}{2}} \int_{\gamma_1}^{\gamma_2}
  I(\theta,\beta,\rho,\varphi,\gamma)\, \mathrm{d}\gamma \,\mathrm{d}\theta
\end{split}
\]
\vspace{-15pt}
\[
  =\frac{KP}{8\pi^2 a b}\biggl[\Bigl[\gamma \sin{\beta} \sin{\varphi}
  \sin(\rho - \theta) + \left(\sin{\gamma} \sin{(\rho - \theta)} \cos{\beta} -
  \cos{\gamma}\\ \cos{\rho - \theta}\right)\cos{\varphi}\Bigr]_{\gamma_1}^{\gamma_2}
\biggr]_{\theta_{\mathrm{N}}-\frac{\psi}{2}}^{\theta_{\mathrm{N}}+\frac{\psi}{2}}\numberthis
\label{eq:north_summer_solstice}
\]
If we now compare this to the warmest section of the orbit for the southern
hemisphere, we will see they are equivalent. 

We will now measure insolation at latitude $-\varphi$ in the southern
hemisphere. The summer solstice here occurs on the opposite side of the orbit, at an orbital
angle of $\theta_{\mathrm{N}}+\pi$. The limits for $\gamma$ are also shifted by
$\pi$ as the sun is on the opposite side of Earth.

Following the same steps as before, we find the insolation for the
southern hemisphere to be
\[
\begin{split}
  Q_{\mathrm{S}} &= \frac{KP}{8\pi^2 a b}\int_{\theta_{\mathrm{N}}-\frac{\psi}{2}}^{\theta_{\mathrm{N}}+\frac{\psi}{2}}
  \int_{\gamma_1}^{\gamma_2} I((\theta+\pi),\beta,\rho,-\varphi,(\gamma+\pi))\,
...
additional $\pi$ multiplying the first term, which disappears once the limits
for $\gamma$ are substituted in. 

More generally, global insolation over a chosen orbital angle range is independent of
where that range lies in the orbit. Applying this to individual latitudes, we
know that insolation over the warmer half of the orbit, going from the vernal to autumnal
equinox,
