...
\begin{figure}
  ...
  \caption{The change of domain used for the Budyko model, latitude $\varphi$
    ...
    domain, which has range $[0,1]$. The albedo of sea is represented by
...
\cite{c_beta}. This can be seen alongside the actual function in Figure
...
\cite{linear_reradiation}. This is gives
\[
  A+BT(y,t),
\]
where, with average cloud cover, $A=202.1$ and $B=1.9$ with a correlation
coefficient of 0.90.
...
$A$                & 0.0167        & Wm$^{-2}$   & North \cite{linear_reradiation}  \\\hline
$B$                & 0.4091        & Wm$^{-2}$\,$^\circ$C$^{-1}$ &  North \cite{linear_reradiation} \\\hline
$C$                & 2.9161        & Wm$^{-2}$\,$^\circ$C$^{-1}$ &  Tung \cite{tung2007topics}\\\hline
$Q_0$              & 341           & Wm$^{-2}$  & Kopp \cite{solar_constant} \\\hline
$T_{\mathrm{ice}}$ & -10           & $^\circ$C         &        \\\hline
$R$                & $4\times10^8$ & Jm$^{-2}$\,$^\circ$C$^{-1}$ &         \\\hline
$\alpha_1$         & 0.32          & None               &          \\\hline
$\alpha_2$         & 0.63          & None               &         
\end{tabular}
\end{table}
