...
Firstly, there will be 2 sets of axes used to describe orientation,
with their origin at either the centre of the sun or the centre of Earth. \textit{The ecliptic frame}
relates to the ecliptic plane, in which Earth's orbit lies. The $x$-axis is
parallel to the orbit's semi-major axis, whilst the $y$-axis is parallel to the semi-minor axis of the
orbit. If these axes are centred on the sun, we can represent
Earth's position using polar coordinates $\theta$ and $r$. \textit{The Earth-fixed
frame}, when centred on Earth, rotates such that the $x$-axis always points
to coordinates $(0^\circ,0^\circ)$, the $y$-axis points to
$(0^\circ,90^\circ)$, and the $z$-axis points to the north
pole. With these axes centred on Earth, we can use spherical
...
to the orbital parameters; obliquity and precession.
...
The orbital parameters, along with the coordinate axes, are shown in
Figure \ref{fig:orbital_params}.
...
\begin{table}
  \centering
  \caption{Earth's orbital parameters in the year 2020.}
...
Precession ($\rho$)          & 2.9101 & $-3.1416$ -  3.1416 & Radians
\end{tabular}
\end{table}
...
a year period. This is due to the cancelling effects of irradiance, which is
proportional to $1/r^2$ and the angular speed, which is also proportional to
$1/r^2$.

In order to express the average yearly insolation as a function of orbital
parameters, we return to the expression for irradiance at a given point,
\[\begin{aligned}
...
\]
This expression is independent of precession, which will be explained later on.
...
\[
  Q(\varepsilon) = \frac{K}{16\pi a^2 \sqrt{1-\varepsilon^2}}.
\]
...
These two functions for insolation can be see in Figure
\ref{fig:yearly_ave_insol_present}. We can see that the functions are even
about the equator, indicating that yearly insolation is always equally
...
$s(\beta,y)$, we can see how obliquity and eccentricity effect the
average yearly insolation. Since $\varepsilon$ only appears in the first
component, expressing the average yearly insolation over the whole globe, we
see that eccentricity will only scale the insolation curve in Figure
\ref{fig:yearly_ave_insol_present}. Conversely, since $\beta$ is only present
in the function $s(y)$, which distributes the insolation according to
latitude, we know that $\beta$ will not effect the magnitude of global
insolation, only the shape of the insolation profile. This can be seen in
Figure \ref{fig:beta_eps_effect}. Here, the range of values given for the orbital
parameters are significantly beyond their actual range, as shown in Table
\ref{tab:orbital_params}, so as to clearly show their impact on
yearly insolation. By increasing
$\beta$, the Earth is tilted further away from upright in the ecliptic
frame. This has the effect of exposing the poles to more direct irradiance
whilst shifting the equator away from it. Since only orientation is changing,
the global average yearly insolation remains the same. Changing eccentricity
only slightly increases the insolation values along the curve, even for
$\varepsilon=0.18$, which is approximately 3 times larger than the maximum
...
\begin{figure}
  ...
  parameter values are $\beta = 0.4091$ and $\varepsilon = 0.0167$.}
