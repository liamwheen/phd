...
\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{budyko_domain.pdf}
...
a constant found using empirical data. 

The average yearly insolation uses the
solar constant, which is the solar irradiance per unit area arriving
at Earth's atmosphere, one astronomical unit from the sun. This
irradiance is spread over the entire surface of Earth, $S = 4\pi r^2$,
where $r$ is the radius of the Earth. However at any given time, Earth only
intercepts the sun's irradiance over an area equal to Earth's cross-section,
$\frac{S}{4}\pi r^2$. Hence the average yearly insolation is the solar
constant divided by 4. In similar work, this has been 343\,Wm$^{-2}$
\cite{widiasih2013,c_beta,insol_latlon} which is based on Tung's work in 2007
\cite{tung2007topics}. However, this is based on a solar constant of
...
$Q_\varepsilon=340.375$, hence $Q_0 = Q_\varepsilon \sqrt{(1-\varepsilon^2)} =
340.327$ given the current value for $\varepsilon$, as shown in Table
...
\cite{c_beta}. This can approximation can be seen alongside the actual function in Figure
\ref{fig:yearly_ave_insol_present}. This is finally multiplied by $(1-\alpha_\eta(y))$, which
describes the degree to which heat is absorbed at a given latitude. This
depends on the albedo of the surface, which in this case is either water or
ice. The albedo over the domain is therefore given as
\[
...
complex, North \textit{et al.} performed linear regression on satellite data to
...
the spacial variable $y$. The only term that relates the temperature
at a given position to the rest of the system is $\bar{T}(t)=\int_0^1 T(y,t)
\,\mathrm{d}y$, which gives the average global temperature.
...
$^\circ$C\,s$^{-1}$, the units for $R$ are J\,m$^{-2}$$^\circ$C\,s$^{-1}$, which
...
derive this expression. The \textit{ecliptic frame}
relates to the ecliptic plane, in which Earth's orbit lies. The $x$-axis points along the
orbit's semi-major axis towards the aphelion (the farthest point from the sun
on Earth's orbit), with the $y$-axis pointing along the semi-minor axis of the
orbit. Using a sun centred coordinate system, we can represent
Earth's position using polar coordinates $\theta$ and $r$. The \textit{Earth-fixed
frame} changes with Earth, according to obliquity and precession. With an
Earth centred coordinate system in this frame, we can use spherical
...
\begin{figure}
  ...
  \ref{tab:orbital_params}. In the second plot, eccentricity is increased to
  0.06, approximately the maximum for Earth's orbit. The third plot shows how
these two cases differ.}
  \label{fig:daily_ave_insol_all_lats}
\end{figure}
Figure \ref{fig:daily_ave_insol_all_lats} shows
how insolation varies over a year period, across the latitudes, using current orbital
...
the aphelion of Earth's orbit, at which point Earth is almost the farthest it
can be from the sun. Summer in the southern hemisphere begins
170 days after the aphelion, when Earth is almost the nearest it can be to the
sun. As the eccentricity of the orbit increases, the perihelion becomes nearer
to the sun whilst the aphelion moves farther away. This leads to a hotter
summer in the southern hemisphere and a cooler summer in the north, as shown in
the difference plot of Figure \ref{fig:daily_ave_insol_all_lats}.

Kepler's second law states
\[
  \frac{\mathrm{d}\theta}{\mathrm{d}t} = \frac{2\pi a b}{Pr^2},
\]
where $P$ is the year period, $r$ is the distance from the sun, and $a$ and $b$
are the semi-major and semi-minor axes respectively. On the scale of a year,
$a$ and $b$ can be considered constant, meaning Earth's angular speed
around the sun is proportional to $1/r^2$.

As eccentricity increases, the distance between the perihelion and the sun
decreases, meaning the angular speed around the perihelion will increase. This is the
cause of the 2 spikes at either pole in the difference plot. These indicate
where the more eccentric orbit has traversed the angular range around the
perihelion faster than our actual orbit. This results in the summer period in
the southern hemisphere lasting less time, becoming comparatively cooler either
side of the warmest 100 days.
\begin{table}[h]
  \centering
...
parameters given in Table \ref{tab:orbital_params}.
\begin{figure}
  \centering
...
\begin{figure}
  \centering
  \includegraphics[width=0.55\linewidth]{solstice_angles.pdf}
...
\[
  ...
  -\sin{\beta}\sin{\varphi} \cos{(\rho -\theta)}.
\]
The insolation over the range $\psi$, centred on the northern summer sosltice is
therefore
\[
...
\begin{align*}
  ...
  &=\int_{\theta_{\mathrm{N}}-\frac{\psi}{2}}^{\theta_{\mathrm{N}}+\frac{\psi}{2}}\int_{\gamma_1^{\mathrm{N}}}^{\gamma_2^{\mathrm{N}}}
  \frac{K}{4\pi r^2}\frac{Pr^2}{2\pi a b}S(\theta,\beta,\rho,\varphi,\gamma)\, \mathrm{d}\gamma\,\mathrm{d}\theta\\
  &=\frac{KP}{8\pi^2 a
  b}\int_{\theta_{\mathrm{N}}-\frac{\psi}{2}}^{\theta_{\mathrm{N}}+\frac{\psi}{2}}
  \int_{\gamma_1^{\mathrm{N}}}^{\gamma_2^{\mathrm{N}}}
...
\begin{figure}
  ...
  \caption{Diagrams of Earth at its perihelion (left) and aphelion (right) from a top-down perspective
    ...
    the day and night on Earth, with an imagined sun in the centre of the
    figure, illuminating the red sections of the circles. The longitudinal boundaries for
    these sections (green) are measured from the prime meridian, pointed to by
  the projection of north pole in the ecliptic plane.}
  \label{fig:gamma_limits}
\end{figure}

If we now compare this to the same orbital angle range, centred around the southern
hemisphere's summer solstice, at latitude $-\varphi$, we will see they are equivalent. 

The southern summer solstice occurs at an orbital angle of $\theta_{\mathrm{N}}+\pi$.
This causes the range over which $S$ is positive to change. Due to symmetry, the circles of
constant latitude in each hemisphere will experience an equal amount of
sunlight at opposite sides of the orbit, as shown in
Figure \ref{fig:gamma_limits}. Since Earth is treated as non-rotating, the prime meridian
always aligns with the projection of the north pole in the ecliptic plane.
Because of this, the limits in the southern hemisphere are offset from the
northern limits by $\pi$, hence we define the southern limits as
$\gamma_1^{\mathrm{S}}=\pi-\gamma_1^{\mathrm{N}}$ and
...
\begin{split}
  ...
  \theta_{\mathrm{N}}+\pi+\frac{\psi}{2}}\int_{\pi-\gamma_1^{\mathrm{N}}}^{\pi-\gamma_2^{\mathrm{N}}}S(\theta,
