...
However, the data analysis by Stephens \cite{equal_hem_albedo} shows that the
net albedo, including cloud coverage, is almost exactly equal. With only
0.2\,Wm$^{-2}$ difference between them. This is due to the increased cloud
cover present in the southern hemisphere counteracting the increased land mass
in the north. 
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{land_distribution.pdf}
  \caption{Distribution of land density across latitude. The reference map
    shown on the right uses a sinusoidal projection in order to preserve area
  as well as equal latitude spacing.}
